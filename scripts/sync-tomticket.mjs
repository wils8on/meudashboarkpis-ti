...
async function sync() {
    ...
    try {
        ...
        console.log(`Camada incremental: ${incremental.detail_requests} detalhe(s), ${incremental.enriched_records} registro(s) enriquecido(s), ${incremental.snapshots} snapshot(s), ${incremental.dimensions} dimensão(ões), ${incremental.quality_issues} inconsistência(s), ${incremental.errors} erro(s), duração ${incremental.duration_ms}ms.`);
    } catch (error) {
