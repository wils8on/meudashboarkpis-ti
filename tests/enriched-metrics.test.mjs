...
test('calcula SLA, resposta, trabalho, avaliação e cobertura', () => {
    const facts = {
        a: buildMetricFact({ id: 'a', creation_date: '2026-08-25T10:00:00Z', end_date: '2026-08-25T15:00:00Z', first_reply_date: '2026-08-25T12:00:00Z', interaction_count: 4, work_time_seconds: 3600, sla: { initialization: { accomplished: true }, deadline: { accomplished: true } }, evaluation: { grade: '5', problem_solved: true }, department: { id: 'd', name: 'TI' } }),
        b: buildMetricFact({ id: 'b', creation_date: '2026-08-25T10:00:00Z', end_date: '2026-08-25T16:00:00Z', first_reply_date: '2026-08-25T14:00:00Z', interaction_count: 8, work_time_seconds: 7200, sla: { initialization: { accomplished: false }, deadline: { accomplished: true } }, evaluation: { grade: '3', problem_solved: false }, department: { id: 'd', name: 'TI' } })
    };
    ...
    assert.equal(metrics.interactions.mean, 6); assert.equal(metrics.evaluation.response_rate, 100); assert.equal(metrics.evaluation.problem_solved_rate, 50); assert.equal(department.mean_interactions, 6); assert.equal(department.mean_evaluation, 4);
});
